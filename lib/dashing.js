...
module.exports.Dashing = function Dashing() {
  ...
  dashing._protected = express.basicAuth(function(user, pass, callback) {
    dashing.authenticateUser(user, pass, function(result) {
      callback(null /* error */, result)
    })
  }, 'Unauthorized Access Prohibited');

  // This can be overridden in server.js to implement custom logic
  var authenticateUser = function (user, pass, callbackResult) {
    var username = 'admin';
    var password = 'admin';

    if (typeof dashing.username != 'undefined') {
      username = dashing.username;
    }
    if (typeof dashing.password != 'undefined') {
      password = dashing.password;
    }

    callbackResult(user == username && pass == password);
  }
